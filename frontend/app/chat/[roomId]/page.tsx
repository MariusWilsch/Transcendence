...
const PrivateRoom: FC<PageProps> = ({ params }: PageProps) => {
	...
	const fetchDataAndSetupSocket = async () => {
		try {
			const userData: User | undefined = !context.userData
			...
			const roomid =
				user !== undefined
					? parseInt(user.intraId) > parseInt(userData.intraId)
						? user.intraId + userData.intraId
						: userData.intraId + user.intraId
					: 1;
			if (
				user === undefined ||
			...
			if (user.intraId && context.socket && userData.intraId) {
				context.socket?.emit('createPrivateRoom', {
					user1: userData.intraId,
					user2: user.intraId,
					clientRoomid: params.roomId,
				});
			}
		} catch (error) {
			setPermission(false);
	...
	const sendPrivateMessage = () => {
		if (
			...
			messageText.trimStart().trimEnd()
		) {
			const cookie = new Cookies();
			const jwt = cookie.get('jwt');
			context.socket.emit('privateChat', {
				to: context.recipientUserId,
				message: messageText.trimStart().trimEnd(),
				jwt,
			});
			setMessages((prevMessages: Message[]) => {
									...
									<div className="flex sm:items-center justify-between  bg-slate-900  border border-[#292D39] rounded ">
										<div className="relative flex items-center space-x-4">
											<div className="p-2">
												{recipient?.status === 'ONLINE' ? (
													<Indicator
														...
														offset={7}
														position="bottom-end"
														color={recipient?.status == 'ONLINE' ? 'green' : 'none'}
														withBorder
													>
														<Avatar size="lg" radius="xl" src={recipient.Avatar} />
													</Indicator>
												) : (
													recipient && (
														<Image
											...
											<div className="flex flex-col leading-tight">
												...
												<span
													style={{ display: recipient?.status == 'ONLINE' ? '' : 'none' }}
									...
									>
										{desplayedMessages?.map(
											(msg: any, index) =>
												...
												(msg.sender !== context.userData?.intraId && recipient ? (
