...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
	...
	async createPrivRoom(
		client: any,
		payload: { jwt: string; user2: string; clientRoomid: string }
	): Promise<void> {
		try {
	...
	async handlePrivateChat(
		...
		try {
			...
			if (recipientSocket) {
				...
				console.log(
					`Private message from ${client.user.intraId} to ${payload.to}: ${message.content}`
				);
			} else {
	...
	async joinChannel(
		client: any,
		payload: { channelId: string; type: string; password: string; jwt: string }
	) {
		try {
			const user = this.chatService.getUserFromJwt(payload.jwt);
			if (!user) {
	...
	async updateChannel(
		...
		try {
			...
			const memberShip = await this.chatService.updateChannelUser(
				client.user.intraId,
				payload.memberId,
				payload.info
			);
			...
			socketRec.map((socket: any) => {
				socket.emit('updateChannelUser', {
					e: `your memberShip at the channel ${memberShip.channelId} has been updated`,
				});
			});
		} catch (e) {
			console.log(e);
